...
def perform_processing(
    locked_pattern: str, floating_patterns: Set[str], excluded_letters: str
) -> Set[str]:
    """
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-l", "--locked_pattern", help="string of known letters in the right position"
    )
    parser.add_argument(
        "-f",
        "--floating_patterns",
    ...
    parser.add_argument(
        "-x", "--excluded_letters", help="string of letters not in the word"
    )
    args = parser.parse_args()

    (
        param_locked_pattern,
        param_floating_patterns,
        param_excluded_letters,
    ) = parse_arguments(
        args.locked_pattern, args.floating_patterns, args.excluded_letters
    )

    if (
        param_locked_pattern is None
        and param_floating_patterns is None
        and param_excluded_letters is None
    ):
        raise Exception(
