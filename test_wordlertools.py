...
class TestRemoveInvalidWords(unittest.TestCase):
    ...
    def test_no_matching_to_remove(self):
        ...
        self.assertSetEqual(
            pattern_processor.remove_invalid_words(
                floating_patterns, self.candidate_words
            ),
            self.candidate_words,
        )
...
class TestIsWordPatternMatch(unittest.TestCase):
    ...
    def test_valid_word_no_excluded_letters(self):
        self.assertTrue(pattern_processor.is_word_a_pattern_match("___l_", "", "apple"))
        self.assertTrue(pattern_processor.is_word_a_pattern_match("___L_", "", "apple"))
        self.assertTrue(pattern_processor.is_word_a_pattern_match("___l_", "", "appLe"))
        self.assertTrue(
            pattern_processor.is_word_a_pattern_match("ch_c__n", "", "chicken")
        )

    def test_valid_word_excluded_letters(self):
        self.assertTrue(
            pattern_processor.is_word_a_pattern_match("___l_", "zxc", "apple")
        )
        self.assertTrue(
            pattern_processor.is_word_a_pattern_match("appl_", "bzcqwtm", "apple")
        )
        self.assertFalse(
            pattern_processor.is_word_a_pattern_match("appl_", "e", "apple")
        )

    def test_invalid_word_no_excluded_letters(self):
        self.assertFalse(
            pattern_processor.is_word_a_pattern_match("___l_", "", "cheese")
        )

    def test_invalid_word_excluded_letters(self):
        self.assertFalse(
            pattern_processor.is_word_a_pattern_match("___l_", "abcdef", "cheese")
        )

    def test_valid_word_excluded_skip_char(self):
        self.assertFalse(
            pattern_processor.is_word_a_pattern_match("___l_", "_", "cheese")
        )

    def test_invalid_word_excluded_skip_char(self):
        self.assertTrue(
            pattern_processor.is_word_a_pattern_match("___l_", "_", "apple")
        )

    def test_empty(self):
        self.assertFalse(pattern_processor.is_word_a_pattern_match("___l_", "abcd", ""))
        self.assertTrue(pattern_processor.is_word_a_pattern_match("___l_", "", "apple"))
        self.assertFalse(pattern_processor.is_word_a_pattern_match("", "abcd", "apple"))
        self.assertFalse(pattern_processor.is_word_a_pattern_match("___l_", "", ""))
        self.assertFalse(pattern_processor.is_word_a_pattern_match("", "abcd", ""))
        self.assertFalse(pattern_processor.is_word_a_pattern_match("", "", "apple"))
        self.assertFalse(pattern_processor.is_word_a_pattern_match("", "", ""))

    def test_pattern_too_small_for_word(self):
        self.assertFalse(
            pattern_processor.is_word_a_pattern_match("___l_", "", "apples")
        )

    def test_pattern_too_big_for_word(self):
        self.assertFalse(
            pattern_processor.is_word_a_pattern_match("___l__", "", "apple")
        )

    def test_pattern_full_word(self):
        self.assertTrue(pattern_processor.is_word_a_pattern_match("apple", "", "apple"))

    def test_full_alphabet_excluded(self):
        self.assertFalse(
            pattern_processor.is_word_a_pattern_match(
...
class TestGenerateLetterPermutations(unittest.TestCase):
    ...
    def test_generate_invalid(self):
        ...
        permutations = pattern_processor.generate_letter_permutations("abcde", 0)
        intended_result = set()
        self.assertSetEqual(permutations, intended_result)

    def test_truncate_when_too_large(self):
        permutations = pattern_processor.generate_letter_permutations("abcdef", 2)
    ...
    def test_get_letters_permutations_distinct(self):
        self.assertListEqual(
            sorted(
                [
                    c
                    for c in pattern_processor.get_letters_for_permutations(
                        {"a___", "_b__"}, "___c", 4
                    )
                ]
            ),
            sorted([c for c in "abc"]),
        )
...
class TestIsLockedOnlyOneLeft(unittest.TestCase):
    ...
    def test_normal(self):
        self.assertTrue(
            pattern_processor.is_locked_only_one_left(
                "a", {"a___", "_a__", "___a"}, "__a_"
            )
        )
        self.assertFalse(
            pattern_processor.is_locked_only_one_left("a", {"a___", "_a__"}, "__a_")
        )

    def test_floating_empty(self):
        self.assertFalse(pattern_processor.is_locked_only_one_left("a", set(), "__a_"))

    def test_locked_empty(self):
        self.assertFalse(
            pattern_processor.is_locked_only_one_left("a", {"a___", "_a__", "___a"}, "")
        )
...
class TestMergePatterns(unittest.TestCase):
    ...
    def setUp(self):
        self.test_permutations = {"_cta", "_tca", "c_ta", "t_ca", "ct_a", "tc_a"}

    def test_merge_all_valid(self):
        self.assertSetEqual(
            pattern_processor.merge_patterns(
                "___a", {"c___", "_t__"}, self.test_permutations
            ),
            {"_cta", "t_ca", "tc_a"},
        )
    ...
    def test_merge_inequal_length(self):
        with self.assertRaises(Exception):
            _ = pattern_processor.merge_patterns(
                "____a", {"c_", "_t_____"}, self.test_permutations
            ), {"_cta", "t_ca", "tc_a"}

        with self.assertRaises(Exception):
            _ = pattern_processor.merge_patterns(
                "a_", {"c_", "_t_____"}, self.test_permutations
            ), {"act_"}

    def test_merge_empty_permutations(self):
        self.assertSetEqual(
            pattern_processor.merge_patterns("_a_", set(), set()), {"_a_"}
        )

    def test_merge_empty_floating(self):
        self.assertSetEqual(
            pattern_processor.merge_patterns("_c__", set(), self.test_permutations),
            {"_cta", "tc_a"},
        )
        self.assertSetEqual(
            pattern_processor.merge_patterns("_c__", {""}, self.test_permutations),
            {"_cta", "tc_a"},
        )

    def test_merge_empty_locked_floating(self):
        self.assertSetEqual(
            pattern_processor.merge_patterns("", set(), self.test_permutations),
    ...
    def test_merge_floating_locked_overlap(self):
        ...
        self.assertSetEqual(
            pattern_processor.merge_patterns("ct__", {"___c"}, permutations), {"ctc_"}
        )

    def test_merge_excludes_floating_match(self):
        permutations = {"o____", "_o___", "__o__", "___o_", "____o"}
        self.assertSetEqual(
            pattern_processor.merge_patterns("", {"__o__"}, permutations),
            {"o____", "_o___", "___o_", "____o"},
        )

    def test_merge_empty_locked(self):
        self.assertSetEqual(
            pattern_processor.merge_patterns(
                "", {"c___", "_t__"}, self.test_permutations
            ),
            {"_cta", "t_ca", "tc_a"},
        )
        self.assertSetEqual(
            pattern_processor.merge_patterns(
                None, {"c___", "_t__"}, self.test_permutations
            ),
            {"_cta", "t_ca", "tc_a"},
        )
    ...
    def test_merge_all_locked_letters(self):
        ...
        self.assertSetEqual(
            pattern_processor.merge_patterns("cate", set(), all_permutation), {"cate"}
        )
...
class TestCalcLetterFrequency(unittest.TestCase):
    ...
    def test_no_floating_or_locked_no_remove(self):
        self.assertDictEqual(
            dict(
                pattern_processor.calc_letter_frequency(
                    self.test_words_for_calc, {""}, "", False
                )
            ),
            {"c": 2, "a": 8, "t": 4, "b": 2, "r": 3, "f": 2},
        )
        self.assertDictEqual(
            dict(
                pattern_processor.calc_letter_frequency(
                    self.test_words_for_calc, {""}, ""
                )
            ),
            {"c": 2, "a": 8, "t": 4, "b": 2, "r": 3, "f": 2},
        )
        self.assertDictEqual(
            dict(
                pattern_processor.calc_letter_frequency(
                    self.test_words_for_calc, {""}, "_____", False
                )
            ),
            {"c": 2, "a": 8, "t": 4, "b": 2, "r": 3, "f": 2},
        )
    ...
    def test_locked_remove(self):
        self.assertDictEqual(
            dict(
                pattern_processor.calc_letter_frequency(
                    self.test_words_for_calc, {""}, "_a_", True
                )
            ),
            {"c": 2, "t": 4, "b": 2, "r": 3, "f": 2},
        )
    ...
    def test_locked_no_remove(self):
        self.assertDictEqual(
            dict(
                pattern_processor.calc_letter_frequency(
                    self.test_words_for_calc, {""}, "_a_", False
                )
            ),
            {"c": 2, "a": 8, "t": 4, "b": 2, "r": 3, "f": 2},
        )
    ...
    def test_floating_and_locked_remove(self):
        self.assertDictEqual(
            dict(
                pattern_processor.calc_letter_frequency(
                    self.test_words_for_calc, {"_a_"}, "_b_", True
                )
            ),
            {"c": 2, "t": 4, "r": 3, "f": 2},
        )
    ...
    def test_over_under_empty_locked_length(self):
        self.assertDictEqual(
            dict(
                pattern_processor.calc_letter_frequency(
                    self.test_words_for_calc, {""}, "_______", False
                )
            ),
            {"c": 2, "a": 8, "t": 4, "b": 2, "r": 3, "f": 2},
        )
        self.assertDictEqual(
            dict(
                pattern_processor.calc_letter_frequency(
                    self.test_words_for_calc, {""}, "___", False
                )
            ),
            {"c": 2, "a": 8, "t": 4, "b": 2, "r": 3, "f": 2},
        )
...
class TestCollectFloatingLetters(unittest.TestCase):
    ...
    def test_collect_patterns_none(self):
        self.assertIsNone(pattern_processor.collect_floating_letters(None, 5), [])
    ...
    def test_collect_more_than_word_length(self):
        with self.assertRaises(Exception):
            pattern_processor.collect_floating_letters({"a_b_", "c_d_", "e_f_"}, 4)
...
class TestProcessAllPatterns(unittest.TestCase):
    ...
    def test_process_all_blank(self):
        ...
        patterns = pattern_processor.process_all_patterns(
            {blank_pattern, blank_pattern, blank_pattern}
        )
        assert patterns is not None
        self.assertListEqual(patterns, [])
...
class TestReducePatterns(unittest.TestCase):
    ...
    def setUp(self):
        self.input_patterns = [{"a": 1, "b": 2}, {"c": 2, "b": 1}, {"a": 4, "z": 1}]
...
class TestGetCandidateWords(unittest.TestCase):
    ...
    def test_get_valid_word_one_option(self):
        self.assertSetEqual(
            pattern_processor.get_candidate_words(
                "_a_t_", {"__n_s"}, "erip", self.valid_words, 5
            ),
            {"nasty"},
        )

    def test_get_valid_word_repeated_character(self):
        possible_words = pattern_processor.get_candidate_words(
            "", {"__o__"}, "", self.valid_words, 5
        )
