import mapper

if __name__ == '__main__':

    # ADD CONFIG HERE #
    floating_letters = ''
    locked_letters = '___ke'
    excluded_letters = 'rtipasghcvnm'

    word_length = 5
    test_data = False
    # END CONFIG #
    
    english_words = mapper.load_words(test_data)
    five_words = mapper.get_words_specified_length(5, english_words)    

    possible_permutations = mapper.generate_letter_permutations(
        floating_letters, word_length)

    # merge permutations and reduce by known letter positions
    valid_permutations = mapper.merge_patterns(locked_letters, possible_permutations)
    candidate_words = mapper.get_words_from_pattern(valid_permutations, excluded_letters, five_words)
    print(candidate_words)
