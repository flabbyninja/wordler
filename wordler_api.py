...
def do_wordler():
  ...
  word_length = 5
  candidate_words = pattern_processor.get_candidate_words(locked_pattern, floating_patterns, excluded_letters, words_file, word_length)

  return jsonify(list(candidate_words))

@app.errorhandler(Exception)
def basic_error(e):
  return jsonify({"error": "Bad parameters " + str(e)}), 500
